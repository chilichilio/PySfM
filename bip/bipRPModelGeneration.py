...
chunk.buildModel(surface_type=Metashape.HeightField, interpolation=Metashape.EnabledInterpolation, face_count=Metashape.HighFaceCount)
...
chunk.buildDem(source_data=Metashape.DataSource.DenseCloudData, interpolation=Metashape.EnabledInterpolation, projection=proj)

chunk.buildOrthomosaic(surface_data=Metashape.DataSource.ElevationData, blending_mode=Metashape.MosaicBlending, projection=proj)
...
chunk.raster_transform.formula = ["B1 * (B3 /  (0.2 * B1 + 0.2 * B2 + 0.2 * B4 + 0.2 * B5 + 0.2 * B6))",
                                  "B2 * (B3 /  (0.2 * B1 + 0.2 * B2 + 0.2 * B4 + 0.2 * B5 + 0.2 * B6))",
                                  "B4 * (B3 /  (0.2 * B1 + 0.2 * B2 + 0.2 * B4 + 0.2 * B5 + 0.2 * B6))",
                                  "B5 * (B3 /  (0.2 * B1 + 0.2 * B2 + 0.2 * B4 + 0.2 * B5 + 0.2 * B6))",
                                  "B6 * (B3 /  (0.2 * B1 + 0.2 * B2 + 0.2 * B4 + 0.2 * B5 + 0.2 * B6))"]

chunk.raster_transform.enabled = True

chunk.exportRaster(path=orthomosaic, raster_transform=Metashape.RasterTransformValue, image_format=Metashape.ImageFormatTIFF,
